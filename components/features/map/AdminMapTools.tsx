...
interface AdminMapToolsProps {
  activeLayer: string | null;
  onLayerChange: (layer: string | null) => void;
  onZoomIn: () => void;
  onZoomOut: () => void;
  onReset: () => void;
  onCreatePin?: () => void; // เพิ่ม prop สำหรับสร้าง Pin
}

export function AdminMapTools({
  activeLayer,
  onLayerChange,
  onZoomIn,
  onZoomOut,
  ...
  return (
    <>
      ...
      <div className="absolute top-26 right-4 z-10 flex flex-col gap-2 bg-white/95 backdrop-blur-sm p-2 rounded-xl shadow-lg border border-gray-100 w-12 items-center">
        {/* ปุ่มสร้าง Pin (เฉพาะ Admin) */}
        <ToolButton
          icon={<MapPin className="w-5 h-5" />}
          label="สร้างหมุด"
          onClick={onCreatePin}
          activeColor="bg-black text-white" // สีดำเด่นๆ
          isActive={false} // ไม่ต้องมี state active ค้างไว้ หรือจะทำเป็น toggle ก็ได้
        />
